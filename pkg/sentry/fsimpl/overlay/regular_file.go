...
func (fd *regularFileFD) Readiness(mask waiter.EventMask) waiter.EventMask {
	...
	wrappedFD, err := fd.getCurrentFD(ctx)
	if err != nil {
		// Just use fd.cachedFD since Readiness can't return an error. This is
		// obviously wrong, but at least consistent with VFS1.
...
func (fd *regularFileFD) EventRegister(e *waiter.Entry, mask waiter.EventMask) {
	...
	if err != nil {
		// Just use fd.cachedFD since EventRegister can't return an error. This
		// is obviously wrong, but at least consistent with VFS1.
