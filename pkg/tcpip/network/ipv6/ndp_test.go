		...
		t.Run(stackTyp.name, func(t *testing.T) {
			for _, test := range tests {
				t.Run(test.name, func(t *testing.T) {
					...
					if p.Route.LinkPacketInfo.RemoteLinkAddress != test.naDstLinkAddr {
						t.Errorf("got p.Route.LinkPacketInfo.RemoteLinkAddress = %s, want = %s", p.Route.LinkPacketInfo.RemoteLinkAddress, test.naDstLinkAddr)
					}
