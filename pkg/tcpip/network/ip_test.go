...
func (t *testObject) DeliverTransportPacket(r stack.NetworkPacketInfo, protocol tcpip.TransportProtocolNumber, pkt *stack.PacketBuffer) stack.TransportPacketDisposition {
...
func TestIPv4ReceiveControl(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			ep.HandlePacket(r.PacketInfo(), truncatedPacket(view, c.trunc, header.IPv4MinimumSize))
...
func TestIPv6ReceiveControl(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			ep.HandlePacket(r.PacketInfo(), truncatedPacket(view, c.trunc, header.IPv6MinimumSize))
			if want := c.expectedCount; nic.tester.controlCalls != want {
				t.Fatalf("Bad number of control calls for %q case: got %v, want %v", c.name, nic.tester.controlCalls, want)
			}
		})
