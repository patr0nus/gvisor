...
func (e *endpoint) WritePacket(r stack.NetworkPacketInfo, gso *stack.GSO, protocol tcpip.NetworkProtocolNumber, pkt *stack.PacketBuffer) *tcpip.Error {
	if e.hdrSize > 0 {
		e.AddHeader(r.LinkPacketInfo.LocalLinkAddress, r.LinkPacketInfo.RemoteLinkAddress, protocol, pkt)
	}
...
func (e *endpoint) sendBatch(batchFD int, batch []*stack.PacketBuffer) (int, *tcpip.Error) {
	...
	for _, pkt := range batch {
		if e.hdrSize > 0 {
			e.AddHeader(pkt.EgressRoute.LinkPacketInfo.LocalLinkAddress, pkt.EgressRoute.LinkPacketInfo.RemoteLinkAddress, pkt.NetworkProtocolNumber, pkt)
		}
