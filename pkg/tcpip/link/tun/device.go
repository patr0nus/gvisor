...
func (d *Device) encodePkt(info *channel.PacketInfo) (buffer.View, bool) {
	...
	if info.Pkt.LinkHeader().View().IsEmpty() && info.Route.LinkPacketInfo.RemoteLinkAddress == "" {
		return nil, false
	}
	...
	if d.hasFlags(linux.IFF_TAP) {
		...
		if info.Pkt.LinkHeader().View().IsEmpty() {
			d.endpoint.AddHeader(info.Route.LinkPacketInfo.LocalLinkAddress, info.Route.LinkPacketInfo.RemoteLinkAddress, info.Proto, info.Pkt)
		}
